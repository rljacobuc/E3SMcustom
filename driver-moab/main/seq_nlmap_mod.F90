...
contains
  ...
  subroutine seq_nlmap_avNormArr(mapper, avp_i, avp_o, lnorm_in, a2s_cons)
    ...
    real(r8), allocatable, dimension(:) :: area_s, area_d
    ...
    lsize_i_moab = mbGetnCells(mapper%src_mbid)
    lsize_o_moab = mbGetnCells(mapper%tgt_mbid)

    if (lsize_i_moab /= lsize_i) then
       write(logunit, '(A,2I8)') 'nlmap> src sizes do not match', &
            lsize_i, lsize_i_moab
       call shr_sys_abort(subname//' ERROR: nlmap> src sizes do not match')
    end if
    if (lsize_o_moab /= lsize_o) then
       write(logunit, '(A,2I8)') 'nlmap> dst sizes do not match', &
            lsize_o, lsize_o_moab
    ...
    if (mapper%nl_conservative) then
       ...
       deallocate(lmins, lmaxs)
        ...
        allocate(area_s(lsize_i), area_d(lsize_o))
        ent_type = mapper%tag_entity_type
       ...
       if (verbose) then
        ...
        deallocate(gmins, gmaxs, glbl_masses, area_s, area_d)
    else
